...
public class EnvMain {
    public static void main(String[] args) {
        ...
        ConfigurableEnvironment environment = genericXmlApplicationContext.getEnvironment(); // <-- ConfigurableApplicationContext 소속 메소드
        ...
        try {
            ...
            propertySources.addLast(new ResourcePropertySource("classpath:member.auth"));
        ...
        String id = environment.getProperty("admin.id");
        String pw = environment.getProperty("admin.pw");
        // 외부파일에 똑같의 key 가 있다면 나중에 생성된 key 로 덮어씌우기 합니다.
        System.out.println(id); // system
        System.out.println(pw); // 123456
        ...
        // GenericXmlApplicationContext gCtx1 = ctx; 이 문법은
        // GenericXmlApplicationContext gCtx = new GenericXmlApplicationContext();
        // 위와 같이 생성 할땐 Environment 를 가져 오지 않습니다.
        // GenericXmlApplicationContext gCtx = (GenericXmlApplicationContext) ctx;
        // 형변환 할때 Environment 를 자동적으로 가져오는 구조 입니다.
        // 위와 같은 구조가 됩니다.
        GenericXmlApplicationContext genericXmlApplicationContext1 = genericXmlApplicationContext;

        genericXmlApplicationContext1.load("classpath:environmentSetting.xml");
        genericXmlApplicationContext1.refresh();

        AdminConnection adminConnection = genericXmlApplicationContext1.getBean("adminConnection", AdminConnection.class);
        // 외부파일에 똑같의 key 가 있다면 나중에 생성된 key 로 덮어씌우기 합니다.
        System.out.println(adminConnection.getAdminId()); // system
        System.out.println(adminConnection.getAdminPw()); // 123456

        genericXmlApplicationContext.close();
        genericXmlApplicationContext1.close();
        System.out.println("\nMain() 종료");
    }
}
