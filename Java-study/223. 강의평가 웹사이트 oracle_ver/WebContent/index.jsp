...
if(request.getParameter("pageNumber") != null) {
	try {
	pageNumber = Integer.parseInt(URLDecoder.decode(request.getParameter("pageNumber"), "UTF-8"));		
	} catch(Exception e) {
		e.printStackTrace();
...
<body>
	...
	<!-- 카드에 나타낼 자료 get -->
		...
		evaluationList = evaluationDAO.getList(lectureDivide, searchType, search, pageNumber);
		
		System.out.println("evaluationList.size() : " + evaluationList.size());
			...
			<div class="card-header bg-light">
				<div class="row">
					<div class="col-8 text-left">
						<%=evaluationList.get(i).getLectureName() %>&nbsp;<small><%=evaluationList.get(i).getUserID() %></small>
					</div>
					<div class="col-4 text-right">
						종합<span style="color: red"><%=evaluationList.get(i).getTotlaScore() %></span>
					</div>
				</div>
			</div>
			<div class="card-body">
				<h5 class="card-title">
					<%=evaluationList.get(i).getEvaluationTitle() %>&nbsp;<small>(<%=evaluationList.get(i).getLectureYear() %>년 <%=evaluationList.get(i).getSemesterDevide() %>)</small>
				</h5>
				<p class="card-text">
				<%=evaluationList.get(i).getEvaluationContent() %>
				</p>
				<div class="row">
					<div class="col-9 text-left">
						성적<span style="color: red;"><%=evaluationList.get(i).getCreditScore() %></span> 널널<span style="color: red;"><%=evaluationList.get(i).getComportableScore() %></span>
						강의<span style="color: red;"><%=evaluationList.get(i).getLectureScore() %></span> <span style="color: green;">(추천: <%=evaluationList.get(i).getLikeCount() %>)</span>
					</div>
					<div class="col-3 text-right">
						<a class="btn btn-success" onclick="return confirm('추천 하시겠습니까?');"
							href="./likeAction.jsp?evaluationID=<%=evaluationList.get(i).getEvaleationID() %>">추천</a> <a
							class="btn btn-warning" onclick="return confirm('삭제 하시겠습니까?');"
							href="./deleteAction.jsp?evaluationID=<%=evaluationList.get(i).getEvaleationID() %>">삭제</a>
					</div>
				</div>
			</div>
		</div>
	...
	<!-- 
		...
		<div class="card bg-light mt-3">
			...
			<div class="card-body">
				...
				<p class="card-text">강의가 많이 널널해서, 솔직히 많이 배운 건 없는 것 같지만 학점도 잘나오고
					너무 좋은것 같습니다.</p>
				<div class="row">
					<div class="col-9 text-left">
				...
				<div class="modal-body">
					<form action="./evaluationRegisterAction.jsp" method="post">
						...
						<div class="form-row">
							...
							<div class="form-group col-sm-4">
								<label>수강학기</label> <select name="semesterDevied"
									class="form-control">
							...
							<div class="form-group col-sm-4">
								<label>강의 구분</label> <select name="lectureDevied"
									class="form-control">
