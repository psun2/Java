...
public class PuyoGame implements GameParameters {
	...
	public void ReleasePair() {
		...
		Block blockA = new Block(blkradius + left_pipe * 2 * blkradius, height / 4, blkradius, colors[color_index]);
		...
		Block blockB = new Block(blkradius + right_pipe * 2 * blkradius, height / 4 + height / 6 * 0, blkradius,
				colors[color_index]);
	...
	private boolean MoveBlock(Block blk) {
		...
		if (!pipes[blk.pipe].blocks.isEmpty()) {
			if (!blk.CollideDown((Block) pipes[blk.pipe].blocks.getLast())) // Check Collision with // last block of the
																			// Pipe
			{
				blk.centerY += 1;
				return true;
	...
	public void RotatePair() {
		...
		case TOP_OR: // top to left
		{
			...
			{// by default we can rotate
				...
				if (!pipes[current_pair.blockA.pipe - 1].blocks.isEmpty()) { // check if the left block is still moving
					if (current_pair.blockA.CollideDown((Block) pipes[current_pair.blockA.pipe - 1].blocks.getLast())) { // check
																															// if
																															// the
																															// right
																															// block
																															...
																															// left
																															// block
						if (current_pair.blockA
								.CollideLeft((Block) pipes[current_pair.blockA.pipe - 1].blocks.getLast())) {
			...
			if (current_pair.blockB.pipe < num_pipes - 1) {
				...
				if (!pipes[current_pair.blockB.pipe + 1].blocks.isEmpty())
					if (current_pair.blockB.CollideDown((Block) pipes[current_pair.blockB.pipe + 1].blocks.getLast()))
						if (current_pair.blockB
								.CollideRight((Block) pipes[current_pair.blockB.pipe + 1].blocks.getLast()))
							rotate__ = false;
