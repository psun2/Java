...
// ----------------------------------------------
	...
	private void gameRender() {
		...
		dbg.drawString("Avrg FPS/UPS: " + df.format(averageFPS) + ", " + df.format(averageUPS), 20, 25); // was (10,55)
...
// use active rendering to put the buffered image on-screen
	 ...
	 * the total frame count, which is the total number of calls to run(); - the
	 * frames skipped in this interval, the total number of frames skipped. A frame
	 * skip is a game update without a corresponding render; - the FPS (frames/sec)
	 * and UPS (updates/sec) for this interval, the average FPS & UPS over the last
	 * NUM_FPSs intervals. The data is collected every MAX_STATS_INTERVAL (1 sec).
	 */
	{
		frameCount++;
		...
		if (statsInterval >= MAX_STATS_INTERVAL) { // record stats every MAX_STATS_INTERVAL
			 ...
			 * timedf.format((double) realElapsedTime/1000L) + "s " + df.format(timingError)
			 * + "% " + frameCount + "c " + framesSkipped + "/" + totalFramesSkipped +
			 * " skip; " + df.format(actualFPS) + " " + df.format(averageFPS) + " afps; " +
